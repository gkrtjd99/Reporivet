...
The operating model is deliberately Sub-Agent-first while keeping coordination durable. For every milestone classified as broad, the common installed-project rule is:
...
- **Main** owns product intent, scope, non-goals, acceptance criteria, the overall hierarchical task tree, dependency state, integration, Plan mutations, and the judgment that evidence is sufficient. Main Skill owns first-Plan creation/resume. Main alone serializes the Plan and may direct the host to dispatch the complete dependency-ready leaf set for each broad milestone concurrently; Reporivet itself does not spawn or dispatch Agents.
- **Implementation leaf** receives one bounded Task Packet with exact reads, allowed writes, protected paths, acceptance criteria, verification commands, stop conditions, and a return schema. If a child is itself broad, only a packet explicitly marked `Role: Task Owner` and `May delegate: yes` may run its predeclared bounded descendants such as `T<n>-A-1`; ordinary leaf Agents do not delegate. Descendants inherit parent scope, protected paths, and acceptance without broadening them.
- **Verification leaf** receives a fresh context and independently inspects the integrated candidate. Read-only verification leaves may run in parallel, cannot delegate by default, and depend on the integrated candidate. Each runs applicable project-owned tests or CI-equivalent commands, checks non-goals and ownership boundaries, maps results to acceptance criteria, and returns findings and residual risks without repairing the candidate.

Concurrent mutable siblings are allowed only with disjoint allowed-write sets, frozen shared interfaces, and separate worktrees. Their results converge on an explicit integration node owned by Main; fresh verification nodes follow that integrated candidate, and any candidate mutation invalidates prior verification evidence. Main records concise returned evidence rather than repeating detailed verification by default. Reporivet installs no Agent spawn/dispatch mechanism, project command runner, CI/deployment engine, scheduler, task store, lease, lock, Gate, evidence archive, hidden state, automatic dispatcher, or other runtime to dispatch, resume, or record this workflow; the Plan and host's native Agent execution carry the procedure.
...
Main Skill creates or resumes the Plan: it searches active and completed history, resumes one matching active Plan, stops on ambiguity, or creates the first ordinary Markdown Plan with the lowest unused current-year ID from the template. Setup and init never create the Plan. Main records the dependency-ready leaves, updates it at checkpoints, and moves a terminal Plan from `active/` to `completed/` with an explicit outcome (`complete`, `cancelled`, or `superseded`). Integration nodes depend on their implementation leaves; fresh verification nodes depend on the integrated candidate. There is no automatic closure, Gate verdict, run directory, scheduler, task store, or second verification engine. Historical Plans remain readable even if they contain retired Gate or evidence fields.
