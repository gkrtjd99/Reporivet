...
- [ ] 잘못된 메모리를 제거하고 사용자 선호를 모델 중립적으로 정리.
...
- Task Lead는 승인된 Task 안의 scheduling·보수 조정·결과 취합만 수행. 범위/acceptance/권한/최종 통합·승인은 Main 소유.
...
상태: 구현 대기. target/paths/evidence/blockers/승인은 구현 결과 후 Main이 기록한다.
...
상태: 구현 대기. independent verdict와 Main acceptance를 구분한다.
