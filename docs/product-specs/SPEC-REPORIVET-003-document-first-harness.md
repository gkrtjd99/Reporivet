...
Every broad-milestone child row retains Owner, State, Depends on, Outcome, and Result and has one matching owner-bearing packet. This rule does not apply to inherently single or serial milestones. The active Plan is the durable resume point; terminal Plans move to `completed/` with an explicit outcome such as complete, cancelled, or superseded. Main owns the overall tree, alone serializes Plan mutations, and does not create a second task database, journal, or closure engine.

Execution is Sub-Agent-first and uses only the host's native Agent execution:

- **Main** owns intent, scope, acceptance, decomposition, the complete dependency-ready leaf set, integration order, Plan updates, and the judgment that returned evidence satisfies the contract. Main dispatches all dependency-ready leaves together rather than iterating over them serially.
- **Implementation leaves** receive bounded Task Packets, change only the assigned project/package paths, and run the smallest applicable project-owned commands. If a child is itself broad, only a packet explicitly marked `Role: Task Owner` and `May delegate: yes` may run its predeclared bounded descendants such as `T<n>-A-1`; ordinary leaf Agents do not delegate. Descendants inherit the parent's scope, protected paths, and acceptance and cannot broaden them.
- **Verification leaves** use a fresh context, independently inspect the integrated candidate, run applicable project-owned verification or CI-equivalent commands, and return criterion-level evidence and residual risks. Read-only verification leaves may run in parallel, do not delegate by default, and depend on the integrated candidate.

Concurrent mutable siblings require disjoint allowed-write sets, frozen shared interfaces, and separate worktrees. Siblings converge on an explicit integration node before fresh verification nodes run. Main integrates the results and records compact evidence rather than repeating detailed verification by default. Reporivet installs no scheduler, task store, lease, lock, automatic dispatcher, copied executor, Gate, or automatic Plan-closure process for this lifecycle.
